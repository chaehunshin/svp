...
def main():
    ...
    for i, video in enumerate(dataloader):
        ...
        save_image(prediction_frames, os.path.join(result_dir, 'generate_image_%03d.jpg'%(0)))
        save_image(prediction_frames, os.path.join(result_dir, 'original_image_%03d.jpg'%(0)))
        ...
        for t in range(1, video.shape[2]):
            ...
            if t < args.n_past:
                ...
                generate_frames = make_grid(predict_frames, nrow=8).cpu().detach()
                original_frames = make_grid(curr_frames, nrow=8).cpu().detach()
                save_image(generate_frames, os.path.join(result_dir, 'generate_image_%03d.jpg'%(t)))
                save_image(original_frames, os.path.join(result_dir, 'original_image_%03d.jpg'%(t)))

                skip = None
                prev_frames = video[:, :, t]
                curr_frames = video[:, :, t+1]
            else:
                predict_frames, prior_state, predictor_state, skip = model.inference(prev_frames, prior_state, predictor_state, skip)
        ...
        grid2gif(os.path.join(result_dir, 'generate_image_*'), os.path.join(result_dir, 'generate_image.gif'), delay=10)
        grid2gif(os.path.join(result_dir, 'original_image_*'), os.path.join(result_dir, 'original_image.gif'), delay=10)
